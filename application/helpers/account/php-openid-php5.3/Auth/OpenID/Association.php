...
class Auth_OpenID_Association {
	 ...
	 * @access private
	 */
	var $assoc_keys = array('version', 'handle', 'secret', 'issued', 'lifetime', 'assoc_type');

	var $_macs = array('HMAC-SHA1' => 'Auth_OpenID_HMACSHA1', 'HMAC-SHA256' => 'Auth_OpenID_HMACSHA256');
	 ...
	 *                            be defined in the future.
	 */
	function Auth_OpenID_Association($handle, $secret, $issued, $lifetime, $assoc_type)
	{
...
{
	$assoc_to_session = array('HMAC-SHA1' => array('DH-SHA1', 'no-encryption'));

	if (Auth_OpenID_HMACSHA256_SUPPORTED)
	{
		$assoc_to_session['HMAC-SHA256'] = array('DH-SHA256', 'no-encryption');
	}
...
{
	if ( ! in_array($session_type, Auth_OpenID_getSessionTypes($assoc_type)))
	{
		return FALSE;
	}
...
function Auth_OpenID_getOnlyEncryptedOrder()
{
	...
	foreach (Auth_OpenID_getDefaultAssociationOrder() as $pair)
	{
		...
		{
			if (Auth_OpenID_HMACSHA256_SUPPORTED && ($assoc == 'HMAC-SHA256'))
			{
				$result[] = $pair;
			}
...
{
	$x = new Auth_OpenID_SessionNegotiator(Auth_OpenID_getOnlyEncryptedOrder());
	return $x;
}
