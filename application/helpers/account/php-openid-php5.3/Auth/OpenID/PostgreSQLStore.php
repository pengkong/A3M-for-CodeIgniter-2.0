...
class Auth_OpenID_PostgreSQLStore extends Auth_OpenID_SQLStore {
	...
	function _set_assoc($server_url, $handle, $secret, $issued, $lifetime, $assoc_type)
	{
		...
		if ($result)
		{
			...
			$this->connection->query($this->sql['set_assoc']['update_assoc'], array($secret, $issued, $lifetime, $assoc_type, $server_url, $handle));
		}
		else
		{
			// Insert a new record because this association wasn't
			// found.
			$this->connection->query($this->sql['set_assoc']['insert_assoc'], array($server_url, $handle, $secret, $issued, $lifetime, $assoc_type));
		}
	}
