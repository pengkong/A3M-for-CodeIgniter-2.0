...
class Auth_OpenID_TrustRoot {
	...
	function buildDiscoveryURL($realm)
	{
		...
		if ($parsed['wildcard'])
		{
			...
			return sprintf('%s://%s%s', $parsed['scheme'], $www_domain, $parsed['path']);
		}
		else
		{
...
function Auth_OpenID_returnToMatches($allowed_return_to_urls, $return_to)
{
	foreach ($allowed_return_to_urls as $allowed_return_to)
	{
		...
		if ( // Parses as a trust root
			($return_realm !== FALSE) && // Does not have a wildcard
			( ! $return_realm['wildcard']) && // Matches the return_to that we passed in with it
...
function Auth_OpenID_getAllowedReturnURLs($relying_party_url, &$fetcher, $discover_function = NULL)
{
	...
	list($rp_url_after_redirects, $endpoints) = Auth_Yadis_getServiceEndpoints($relying_party_url, $xrds_parse_cb, $discover_function, $fetcher);
	...
	call_user_func_array($discover_function, array($relying_party_url, $fetcher));
