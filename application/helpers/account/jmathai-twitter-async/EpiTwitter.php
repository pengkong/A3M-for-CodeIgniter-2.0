...
class EpiTwitter extends EpiOAuth {
	...
	{
		if (preg_match('@^/search[./]?(?=(json|daily|current|weekly))@', $endpoint)) return "{$this->searchUrl}{$endpoint}";
		elseif ( ! empty($this->apiVersion)) return "{$this->apiVersionedUrl}/{$this->apiVersion}{$endpoint}";
		else
	...
	private function request($method, $endpoint, $params = NULL)
	{
		...
		if ( ! $this->isAsynchronous) $resp->response;
	...
	private function request_basic($method, $endpoint, $params = NULL, $username = NULL, $password = NULL)
	{
		...
		{
			if ($this->isMultipart($params)) curl_setopt($ch, CURLOPT_POSTFIELDS, $params);
			else
...
class EpiTwitterJson implements ArrayAccess, Countable, IteratorAggregate {
	...
	public function __get($name)
	{
		...
		if (isset($accessible[$name]) && $accessible[$name]) return $this->$name;
		elseif (($this->code < 200 || $this->code >= 400) && ! isset($accessible[$name])) EpiTwitterException::raise($this->__resp, $this->debug);
